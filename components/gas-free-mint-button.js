...
export function GasFreeMintButton (props) {
    ...
    React.useEffect(() => {
      const tokenId = Number(mintedData);
      if (tokenId !== NaN && tokenId !== 0) {
    ...
    const onClickMinted = () => {
      window.open(`https://testnets.opensea.io/ja/assets/goerli/${props.contractAddress}/${tokenId}`);
    }
    ...
    {alreadyMinted && (
      <button
        ...
        className="button"
        data-mint-started={isMintLoading}
